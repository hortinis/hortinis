...
| Deletion propagates and stale clients cannot resurrect an identity | ADR-0024 | `DeleteTechnicalRecordOperation`, `TechnicalTombstone`; `generation-bound-delete.json` and `snapshot-final-page.json` | G2; G5 |
| Interrupted incremental exchange can resume without advancing past unapplied work | ADR-0010 and ADR-0023 | Existing opaque `SyncCursor` and `ChangePage` | G2; G5 |
...
Cross-runtime conformance currently covers the V0 corpus. Contract tests validate the G1 policy fixtures
against generated schemas. G2 through G4 add TypeScript and Java consumers with each behavior; G5 runs
