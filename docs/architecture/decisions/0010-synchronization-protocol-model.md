...
- Give every submitted operation a stable idempotency identifier. A retry must reuse the same identifier
  and validated request fields; reusing an identifier with a different request is an error.
- Compare validated operation requests structurally by their typed fields. Clients may continue to submit
  ordinary JSON with arbitrary whitespace and property order; a reused operation identifier with any
  different validated field is an error.
...
- The service must atomically persist idempotency, accepted state, the resulting revision and sequence, and the change journal. It must reject the same idempotency identifier with different validated request fields.
